...
```{r}
sustitucion_adelante <- function(L, b){
  n <- nrow(L)
  x <- numeric(n)
  for(i in 1:n){
...
sustitucion_atras <- function(U, b){
  n <- nrow(U)
  x <- numeric(n)
  for(i in n:1){
...
print(sustitucion_adelante(L,b1))
cat("Resultado sustitución hacia atrás:\n")
print(sustitucion_atras(U,b2))
```


## 2) Eliminación Gaussiana con pivoteo parcial escalado
...
## 3) Precisión completa vs 4 dígitos
